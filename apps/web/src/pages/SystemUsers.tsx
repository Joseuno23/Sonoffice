              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1020 }}>
                ...
                <tbody>{pageRows.map((user) => (
                  <tr key={user.id} style={{ borderBottom: '1px solid var(--border,#e5e8ec)', transition: 'background .12s' }} onMouseEnter={(event) => (event.currentTarget.style.background = 'var(--surface-2,#f7f8fa)')} onMouseLeave={(event) => (event.currentTarget.style.background = 'transparent')}>
                    ...
                    <td style={{ padding: '13px 16px', textAlign: 'right' }}><button onClick={(event) => { const rect = event.currentTarget.getBoundingClientRect(); setUserAction({ id: user.id, x: rect.right, y: rect.bottom }); }} style={{ width: 30, height: 30, border: 'none', background: 'transparent', borderRadius: 8, color: 'var(--faint,#94a3b8)', cursor: 'pointer', display: 'inline-grid', placeItems: 'center', transition: 'all .14s' }}><Icon d="M12 6h.01M12 12h.01M12 18h.01" size={17} sw={2} /></button></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
