...
export default function CostOrderForm() {
  ...
  // Servicios según tipo (I/E).
  useEffect(() => {
    api.getCostOrderServices(tipo).then((res) => setServicios(res?.success ? res.data || [] : [])).catch(() => setServicios([]));
    setIdServicio('');
  }, [tipo]);

  // Campañas + productos según cliente.
  useEffect(() => {
    if (!cliente) { setCampanas([]); setProductos([]); setIdCampana(''); setIdProducto(''); return; }
  ...
  const canSubmit = !!cliente && !!proveedor && !!idServicio && !!idCampana && !!idProducto && validDetails.length > 0;
  ...
  const submit = () => {
    ...
    api.createCostOrder({
      ...
      .then((res) => {
        if (res?.success) {
          setMessage({ type: 'success', text: res.message || 'Orden creada.' });
          setTimeout(() => navigate('/medios/ordenes-costo/listar'), 900);
        } else {
          setMessage({ type: 'error', text: res?.message || 'No se pudo crear la orden.' });
        }
      })
      .catch(() => setMessage({ type: 'error', text: 'No se pudo crear la orden.' }))
    ...
    <div style={{ animation: 'scfade .35s ease', maxWidth: 1180, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20, flexWrap: 'wrap', marginBottom: 22 }}>
        <div>
          ...
          <h1 style={{ fontSize: '1.5rem', fontWeight: 700, letterSpacing: '-.02em', margin: '0 0 5px', color: 'var(--fg,#0f172a)' }}>Nueva orden de costo</h1>
          <p style={{ margin: 0, color: 'var(--muted,#64748b)', fontSize: 14 }}>Completa la información y agrega el detalle de la orden.</p>
        </div>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/medios/ordenes-costo/listar')} style={btnGhost}><Icon d="M18 6L6 18M6 6l12 12" size={16} sw={2} />Cancelar</button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20, minWidth: 0 }}>
          <div style={card}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 18px' }}>
              <SearchSelect value={cliente} label="Cliente" placeholder="Selecciona un cliente" fetcher={api.getCostOrderClients} onSelect={setCliente} />
              ...
              <div>
                ...
                <select value={tipo} onChange={(e) => setTipo(e.target.value as 'I' | 'E')} style={inBase}>
